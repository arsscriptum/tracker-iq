...
TrackerType string_to_tracker_type(const std::string& str) {
	static const std::unordered_map<std::string, TrackerType> trackerMap = {
		{"All", TrackerType::All},
		{"UDP", TrackerType::UDP},
		{"WinSock", TrackerType::WinSock},
		{"IP", TrackerType::IP},
		{"Archived", TrackerType::Archived},
		{"Best", TrackerType::Best},
		{"Test", TrackerType::Test}
	};

	auto it = trackerMap.find(str);
	if (it != trackerMap.end()) {
		return it->second;
	}

	return TrackerType::All; // Default case if not found
}

// Function to convert TrackerType enum to string
std::string tracker_type_to_string(TrackerType type) {
	switch (type) {
	case TrackerType::All: return "All";
	case TrackerType::UDP: return "UDP";
	case TrackerType::WinSock: return "WinSock";
	case TrackerType::IP: return "IP";
	case TrackerType::Archived: return "Archived";
	case TrackerType::Best: return "Best";
	case TrackerType::Test: return "Test";
...
void banner() {
	std::wcout << std::endl;
	COUTBANNER_1("tracker-iq v2.1 - benchmarking tool for torrent trackers and indexers\n");
	COUTBANNER_2("Built on %s\n", __TIMESTAMP__);
	COUTBANNER_2("Copyright (C) 2000-2021 Guillaume Plante\n");
	std::wcout << std::endl;
}
void usage() {
	COUTBANNER_2("Usage: tiq [-h][-d][-v][-n][-u] url [-p] path \n");
	COUTBANNER_2("   -v                    Verbose mode\n");
	COUTBANNER_2("   -h                    Help\n");
	COUTBANNER_2("   -n                    No banner\n");
	COUTBANNER_2("   -t                    Timeout in seconds\n");
	COUTBANNER_2("   -c, --config  <file>  Specify config  file\n");
	COUTBANNER_2("   -d, --download        Download the latest version of trackers\n");
	COUTBANNER_2("   -j, --json            Json  format\n");
	COUTBANNER_2("   -i, --indexer <url>   Rate Indexer\n");
	COUTBANNER_2("   -u, --url     <url>   test a single tracker url\n");
	COUTBANNER_2("   -p, --path    <file>	 Path of the file containing all trackers\n");
	std::wcout << std::endl;
}
...
std::string get_trackers_index_url(TrackerType type) {

	switch (type) {
		case TrackerType::All: 
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/all.txt"; 
		break;
		case TrackerType::UDP: 
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/udp.txt"; 
		break;
		case TrackerType::WinSock: 
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/ws.txt"; 
		break;
		case TrackerType::IP:
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/ip.txt"; 
		break;
		case TrackerType::Archived: 
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/archive.txt"; 
		break;
		case TrackerType::Best:
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/best.txt";
			break;
		case TrackerType::Test:
			return "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/test.txt";
			break;
		default: 
			return  "https://raw.githubusercontent.com/arsscriptum/trackerslist/refs/heads/latest/index/all.txt";
		break;
