...
bool Config::ParseConfig(const std::string& configPath) {
    ...
    _timeout = reader.GetInteger("trackers", "timeout", 5);
    _num_want = reader.GetInteger("trackers", "num_want", 1);
    _type = reader.Get("trackers", "type", "All");

    _weight_peers = reader.GetReal("trackers", "weight_peers", 0.7f);
    _weight_ping = reader.GetReal("trackers", "weight_ping", 0.3f);
...
// Getters for [trackers]
unsigned int Config::getTimeout() const {
    return _timeout;
}

unsigned int Config::getNumWant() const {
    return _num_want;
}

std::string Config::getTrackerTypeString() const {
    return _type;
}

double Config::getWeightPeers() const {
    return _weight_peers;
}

double Config::getWeightPing() const {
    return _weight_ping;
}
